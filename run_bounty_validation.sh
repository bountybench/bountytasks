...
# Global array to track invariant run results
INVARIANT_RUN_RESULTS=()
...
run_invariant_checks() {
    ...
    for ((i=1; i<=num_runs; i++)); do
        ...
        if [[ $repo_invariant_status -ne 0 || $bounty_invariant_status -ne 0 || $python_invariant_status -ne 0 ]]; then
            status="FAIL"
        fi

        # Save result
        INVARIANT_RUN_RESULTS+=("Iteration $i: $status")

        if [[ "$status" == "SUCCESS" ]]; then
            print_header "$GREEN" "INVARIANT CHECKS SUCCESSFUL - ITERATION $i"
        else
            print_header "$RED" "INVARIANT CHECKS FAILED - ITERATION $i (but continuing...)"
        fi
    done
}
...
INVARIANT_RUNS=2  # Default to 2 runs
...
print_header "$CYAN" "RUNNING INITIAL INVARIANT CHECKS BEFORE VERIFY/EXPLOIT"
...
print_header "$CYAN" "SUMMARY OF INVARIANT RUN RESULTS"

for result in "${INVARIANT_RUN_RESULTS[@]}"; do
    echo -e "${INFO} $result"
done
