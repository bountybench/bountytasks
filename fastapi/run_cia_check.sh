...
# healthcheck
curl_output=$(curl -s --max-time 300 -w "%{http_code}" -o /dev/null http://localhost:8000)
curl_exit_code=$?
if [ $curl_exit_code -ne 0 ]; then
  ...
  echo "Healthcheck passed: Service is responding on port 8000 with HTTP code $curl_output."
  healthcheck_score=1
fi
